...
void write_str_to_path(char *path, char *str) {
  FILE *f = fopen(path, "w");
  if (!f)
    fprintf(stderr, "fopen error when trying to write to %s\n", path);
  if (fwrite(str, sizeof(char), strlen(str), f) != strlen(str))
    fprintf(stderr, "fwrite error when trying to write to %s\n", path);
  fclose(f);
}
...
void focus(Client *c) {
  ...
  if (c) {
    ...
    // statically alloced, so c->name will always be true.
    write_str_to_path(FOCUS_TITLE, c->name);

    if (ch.res_class) {
      write_str_to_path(FOCUS_CLASS, ch.res_class);
      XFree(ch.res_class);
    } else {
      // write an empty string.
      write_str_to_path(FOCUS_CLASS, empty_string);
    }
    if (ch.res_name) {
      XFree(ch.res_name);
    }
...
void keypress(XEvent *e) {
  ...
  for (i = 0; i < LENGTH(keys); i++)
    if (keysym == keys[i].keysym &&
      ...
      printf("Shortcut key pressed: %d\n", i);
      printf("Executing shortcut: %p\n", keys[i].func);
...
void runAutostart(void) {
  /// HANDLE ALL THE RUN ONCE STUFF
  FILE *file;                           // Declare a file pointer
  char filename[50] = "/tmp/dwmstart-"; // Specify the filename
                                        //
  pid_t pid = getpid();                 // Get the PID of the current process

  // Append the PID to the filename
  sprintf(filename, "%s%d", filename, pid);

  // if the file exists, then we've already started up, so return
  ...
  printf("Creating file '%s', and running the autostart.\n", filename);
  ...
  if (file == NULL) {
    ...
    fprintf(file, "%d has started up!!", pid);

    // Close the file
    fclose(file);
  }

  /// THE ACTUAL AUTOSTART STUFF
  // log the start and end, so we know which command fails if dwm fails.
#define R(cmd)                                                                 \
  printf("running " cmd "\n");                                                 \
  system(cmd " &");                                                            \
  ...
  // setup
  // allow root access to X programs, eg xclip
  R("xhost +si:localuser:root");

  // X daemons
  R("killall -q dwmblocks; dwmblocks");
  R("dunst");
  R("buckle");
  R("feh --bg-fill /home/zack/Pictures/nepeta.gif");
  R("slstatus");
  // resize and shape monitors when the monitor setup changes.
  R("x-on-resize -c user_xorg_setup");
  // and run it once at startup, in case the monitors are already plugged in.
  R("user_xorg_setup");
...
#define SPAWN_LOG_PATH "/home/zack/.config/dwm_spawn_log.txt"
...
void spawn(const Arg *arg) {
  if (fork() == 0) {
    ...
    // Open a file for stdout redirection (overwrites the file)
    FILE *file = freopen(SPAWN_LOG_PATH, "a", stdout);
    if (file == NULL) {
      fprintf(stderr, "Failed to open logfile.txt for writing\n");
      exit(EXIT_FAILURE);
    }

    freopen(SPAWN_LOG_PATH, "a", stderr);
...
/// define wrappers around spawnterm for IPC purposes.
// takes in empty arg array
void spawnterm(const Arg *arg) {
  // just spawn the terminal using the default command
  Arg a = SHCMD("st");
  spawn(&a);
}

void spawnipc(const Arg *arg) {
  // just spawn the terminal using the default command
  Arg a = SHCMD("");
  spawn(&a);
}

void spawntermipc(const Arg *arg) {
  // just spawn the terminal using the default command
  Arg a = SHCMD_TERM("st -e ipc");
  spawn(&a);
}
...
// log all the startup script executions.
#define LOGGER_PATH "/usr/local/bin/log"

void run_scripts_in_dwm_d() {
  const char *dwm_d_path = CONFIG_DIR;

  DIR *dir = opendir(dwm_d_path);
  if (dir == NULL) {
    fprintf(stderr,
            "Failed to open dwm.d directory, not running any startups.\n");
    return;
  }
  ...
  while ((entry = readdir(dir)) != NULL) {
    if (entry->d_type == DT_REG) { // Only process regular files
      snprintf(script_path, sizeof(script_path), "%s/%s", dwm_d_path,
               entry->d_name);

      printf("Spawning startup script: %s\n", script_path);

      Arg args = SHCMD(script_path);
      spawn(&args);

      printf("Spawned startup script: %s\n", script_path);
    }
  }
...
int main(int argc, char *argv[]) {
  ...
  runAutostart();
  run();
  // if SIGHUP, just run itself again.
  if (restart)
    execvp(argv[0], argv);
  // Delete the temporary file if you want to run the "run-once" code again next
  // time
  cleanup();
  XCloseDisplay(dpy);
  return EXIT_SUCCESS;
